...
const AppPasswordInput: React.FC<{ value: string, onChange: (val: string) => void }> = ({ onChange, value }) => {
    ...
    return (
        <div className='border rounded-md flex pl-4'>
            <IonInput name='password' type={hidden ? "password" : "text"} required value={value} onIonInput={(e) => onChange(e.detail.value as string)} onIonChange={(e) => onChange(e.detail.value as string)} fill='outline' className='  ' />
